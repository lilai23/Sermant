 ...
 * @since 2024-08-29
 */
public class DefaultServerAttributesExtractor<REQUEST, RESPONSE, GETTER extends OtelServerAttributesGetter<REQUEST, RESPONSE>> implements
        OtelAttributesExtractor<REQUEST, RESPONSE> {
    private GETTER getter;

    public DefaultServerAttributesExtractor(GETTER getter) {
        this.getter = getter;
    }

    @Override
    public void onStart(AttributesBuilder attributes, Context parentContext, REQUEST request) {
        attributes.put(ClientAttributes.CLIENT_ADDRESS, getter.getClientAddress(request));
        attributes.put(ClientAttributes.CLIENT_PORT, getter.getClientPort(request));
    }
